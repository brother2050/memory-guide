...
<details><summary>查看答案</summary>B. 单例模式确保一个类只有一个实例。工厂方法是根据参数创建不同对象，建造者是构建步骤复杂的对象。</details>

**2. 当接口不兼容时，应该使用哪种结构型模式？**
...
<details><summary>查看答案</summary>C. 适配器模式用于接口不兼容的场景。代理模式控制对象访问，装饰器模式动态添加功能，外观模式简化复杂子系统。</details>
...
<details><summary>查看答案</summary>B. 观察者模式实现一对多通知，当一个对象状态改变时通知所有观察者。策略模式是算法可替换，状态模式是行为随状态变。</details>

### ⚠️ 常见错误

| 错误 | 表现 | 正确做法 |
|------|------|---------|
| 不理解场景就记模式 | 背了23种模式却不知道什么时候用 | 先理解"问题场景"，再匹配模式：需要单例→单例模式 |
| 不画UML图 | 只记文字描述，类之间的关系模糊 | 每个模式画出类图，标注继承、组合、依赖关系 |
| 混淆相似模式 | 策略和状态、代理和装饰器分不清 | 对比两个模式的区别：策略是算法替换，状态是行为随状态变 |
| 过度设计 | 简单问题也要套模式 | 先用最简单的方案，只有当复杂度增加时才引入模式 |
| 不读源码 | 只看教科书的例子，不看实际框架中的应用 | 在Spring、React等框架中找模式的实际应用案例 |

...
## 📍 导航
> [返回编程记忆编码](INDEX.md) | [返回主页](../README.md)
...
| 错误 | 表现 | 正确做法 |
|------|------|---------|
| 死记代码不理解逻辑 | 背诵排序代码却说不清为什么这样写 | 先理解算法思想，再用伪代码描述，最后写实现 |
| 不画流程图 | 脑中想象递归调用却越想越乱 | 用树形图/流程图展开递归过程，可视化每一步 |
| 只看不实操 | 看懂了觉得"会了"，一写就卡壳 | 每学一个算法必须手写实现+调试通过 |
| 忽略边界条件 | 快速排序在已排序数组上退化为O(n²) | 分析最好/最坏/平均情况，测试边界用例 |
| 混淆时间复杂度 | 分不清O(n log n)和O(n²)的实际差距 | 用具体数字感受：n=1000时，n²=100万 vs n log n≈1万 |
...
2. 算法记忆中，O(n²)复杂度通常对应什么类型的算法？
   - A. 分治算法  B. 双重循环（如冒泡排序）  C. 递归算法  D. 贪心算法
   答案：B

3. 算法记忆的核心方法是什么？
   - A. 背诵代码  B. 算法名→核心思想→画面编码→复杂度锚定  C. 只做题  D. 看视频
   答案：B

...
| 错误 | 表现 | 正确做法 |
|------|------|---------|
| 不写代码只看 | 看语法书觉得"懂了"，一写就报错 | 每学一个语法必须写3个以上示例代码并运行 |
| 不对比相似语法 | `==`和`===`、`let`和`var`分不清 | 制作对比表格，标注每个语法的适用场景和区别 |
| 忽略语言差异 | 用Python的思维写Java代码 | 学习新语言时先总结与已知语言的差异点 |
| 死记语法不理解设计 | 背了`async/await`的写法却不知道为什么需要它 | 理解语法解决的问题：回调地狱→Promise→async/await |
| 不关注版本更新 | 还在用ES5写JavaScript | 关注语言新版本特性，了解现代最佳实践 |
...
1. 编程语法记忆的核心编码方式是什么？
   - A. 背诵语法书  B. 功能场景→关键词锁→谐音锚点→代码画面  C. 只敲代码  D. 看文档
   答案：B

2. 从入门到掌握30个核心语法通常需要多少天？
   - A. 1天  B. 3天  C. 4-7天  D. 1个月
   答案：C

3. 易混语法的最佳记忆方法是什么？
   - A. 分开记忆  B. 对比记忆+实操练习  C. 只记一个  D. 背诵区别表格
   答案：B
